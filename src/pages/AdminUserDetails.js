...
const AdminUserDetails = () =>{
    ...
    const postsFirstBatch = async () => {
        try {
            const data = await getDocs(query(collection(db, "users") , orderBy("email", "desc"), limit(5)));
            // console.log(data);
          let posts = [];
          let lastKey = "";
      ...
      const postsNextBatch = async (key) => {
        try {
          const data = await getDocs(query(collection(db, "users")  , orderBy("email", "desc"), limit(5), startAfter(key)))
            ...
            <Container fluid className="p-0 overflow-hidden">
                ...
                <Row className="g-0">
                    <h2 className="text-center fs-4 shadow-lg p-2 mb-3 bg-body rounded mt-4"
                        ...
                        backgroundImage: "linear-gradient( 174.2deg,  #C2EDCE 7.1%, #BADFE7 67.4% )"}}>Customer Detail</h2>
                </Row>
